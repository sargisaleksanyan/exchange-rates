...
def find_update_date(soup: BeautifulSoup):
    updated_at_element = soup.find("p", string=re.compile(r"Last updated:"))

    if updated_at_element is not None:
        updated_at_text = updated_at_element.get_text()
        if updated_at_text is not None:
            updated_at_text = updated_at_text.replace(UPDATED_AT_STRING, "").replace("Date:", "").replace(
                "Time:").replace("  ", " ").strip()
...
def extract_exchange_rates_from_table(table: PageElement) -> List[ExchangeRate]:
    ...
    if (thead is not None):
        ...
        if thead_text is not None and 'bank buys'.lower() in thead_text and 'bank sells' in thead_text and 'currencies' in thead_text:
...
def get_rates_from_dubai_islamic_bank():
    content = make_get_request_with_proxy(BankExchangeRateUrl.DUBAI_ISLAMIC_BANK)
    if (content is not None):
        ...
        if (soup is not None):
            ...
            if tables is not None and len(tables) > 0:
                for table in tables:
                    exchange_rates_from_table = extract_exchange_rates_from_table(table)
                    exchange_rates = exchange_rates + exchange_rates_from_table
            ...
            company_exchange_rates.set_exchange_type(ExchangeType.CASH) #TODO from there website it is not clear if exchange rate is for transfer or cash
            return company_exchange_rates

    return None


def scrape_dubai_islamic_bank() -> ExchangeCompany | None:
    try:
        company_exchange_rates = get_rates_from_dubai_islamic_bank()
        exchange_company = ExchangeCompany(BankName.DUBAI_ISLAMIC_BANK,
                                           BankUrl.DUBAI_ISLAMIC_BANK,
